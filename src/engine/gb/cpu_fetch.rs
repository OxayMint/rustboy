    ...
    pub fn fetch_data(&mut self) {
        ...
        self.fetched_data = match self.current_instruction.address_mode {
            ...
            AddressMode::D16 | AddressMode::R_D16 => {
                let low = self.bus.read8(self.regs.pc as usize) as u16;
                self.emu_cycles(1);
                let hi = self.bus.read8(self.regs.pc as usize + 1) as u16;
                self.emu_cycles(1);
            ...
            AddressMode::A8_R => {
                self.mem_dest = 0xFF00 | self.bus.read8(self.regs.pc as usize) as usize;
                self.destination_is_mem = true;
                self.emu_cycles(1);
                self.increment_pointer(1);
                0
            ...
            AddressMode::A16_R | AddressMode::D16_R => {
                let low = self.bus.read8(pc);
                self.emu_cycles(1);
                let hi = self.bus.read8(pc + 1);
                self.emu_cycles(1);
            ...
            AddressMode::MR => {
                ...
                let res = self.bus.read8(addr) as u16;
                self.emu_cycles(1);
            ...
            AddressMode::R_A16 => {
                let low = self.bus.read8(pc);
                self.emu_cycles(1);
                let hi = self.bus.read8(pc.wrapping_add(1));
                self.emu_cycles(1);
